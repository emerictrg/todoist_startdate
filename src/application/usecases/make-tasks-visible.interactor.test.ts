...
const getMockedGateway = 
    (isSuccessfull: boolean = true): interactor.DataGateway => {
        return {
            saveState: jest.fn((tasks: Task[]) => {
...
describe('Activate Tasks Interactor', () => {
    test('is activating all the tasks', async () => {
        ...
        const response: interactor.ResponseModel =
            await usecase.changeVisibility(request);
    ...
    test('throw an error if the update is not successful', async () => {
        ...
        await expect(usecase.changeVisibility(request))
            .rejects.toThrow();
    });
});
